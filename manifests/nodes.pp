...
{id => 'a.simizu',passwd => 'sizu2168',group => 'Managers',staff_name => '清水　敦'},
...
node /^s1225\-tab-2.*/ inherits default{
  include office
  class { "ricoh":
    printer_ip => '192.168.71.164'
  }
...
node /^s1307\-tab.*/ inherits default{
  ...
  class { 'user-add2':
    users => [
      ...
      {id => 'y.katsuyama',passwd => '4Dap9124i',group => 'Managers',staff_name => '勝山　芳孝'},
...
node /^s1151\-tab.*/ inherits default{
  ...
  class { 'user-add2':
    users => [
      {id => 'j.izumi ',passwd => '20Llc9pn',group => 'Managers',staff_name => '泉 潤'},
      {id => 'staff-1151', passwd => '',staff_name => '倶知安店スタッフ'}
    ]
  }
}
...
node /^s1152\-tab.*/ inherits default{
  ...
  class { 'user-add2':
    users => [
      ...
      {id => 'staff-1152', passwd => '',staff_name => '余市店スタッフ'}
    ]
  }
}
...
node /^s1153\-tab.*/ inherits default{
  ...
  class { 'user-add2':
    users => [
      {id => 'n.kayukawa',passwd => 'k10y563',group => 'Managers',staff_name => '粥川　紀夫'},
      {id => 't.watanabe',passwd => 'Epz2paqw',group => 'Managers',staff_name => '渡辺　哲平'},
...
node /^s1154\-tab.*/ inherits default{
  include eset
  include office
  class { "ricoh":
    printer_ip => '192.168.65.197'
  }
  class { 'user-add2':
    users => [  
      {id => 't.takahasi',passwd => 't12k828',group => 'Managers',staff_name => '高橋　吏'},
      {id => 'staff-1154', passwd => '',staff_name => '紋別店スタッフ'}
    ]
  }
}
